...
func SetExecutablePath(path string) error {
	...
	if gitVersion.LessThan(versionRequired) {
		...
		return fmt.Errorf("installed git version %q is not supported, GitBundle requires git version >= %q, %s", gitVersion.Original(), GitVersionRequired, moreHint)
	}
...
func HomeDir() string {
	if setting.Git.HomePath == "" {
		...
		// attention: when the git module is called in gitbundle sub-command (serv/hook), the log module is not able to show messages to users.
		// for example: if there is gitbundle git hook code calling git.NewCommand before git.InitXxx, the integration test won't show the real failure reasons.
...
func syncGitConfig() (err error) {
	...
	for configKey, defaultValue := range map[string]string{
		"user.name":  "GitBundle",
		"user.email": "gitbundle@fake.local",
	} {
	...
	// Set git some configurations - these must be set to these values for gitbundle to work correctly
	...
	// however, some docker users and samba users find it difficult to configure their systems so that GitBundle's git repositories are owned by the GitBundle user. (Possibly Windows Service users - but ownership in this case should really be set correctly on the filesystem.)
	...
	// As GitBundle now always use its internal git config file, and access to the git repositories is managed through GitBundle,
