...
func getWorkPath(appPath string) string {
	...
	if giteaWorkPath, ok := os.LookupEnv("GITBUNDLE_WORK_DIR"); ok {
...
func init() {
	...
	log.NewLogger(0, "console", "console", fmt.Sprintf(`{"level": "info", "colorize": %t, "stacktraceLevel": "none"}`, log.CanColorStdout), IsProd)
...
// GITBUNDLE_CUSTOM environment variable and with provided overrides before stepping
// back to the default
func SetCustomPathAndConf(providedCustom, providedConf, providedWorkPath string) {
	if len(providedWorkPath) != 0 {
		AppWorkPath = filepath.ToSlash(providedWorkPath)
	}
	if giteaCustom, ok := os.LookupEnv("GITBUNDLE_CUSTOM"); ok {
...
func loadFromConf(allowEmpty bool, extraConfig string) {
	...
	AppName = Cfg.Section("").Key("APP_NAME").MustString("GitBundle: A modern git service")
	...
	SSH.TrustedUserCAKeysFile = sec.Key("SSH_TRUSTED_USER_CA_KEYS_FILENAME").MustString(filepath.Join(SSH.RootPath, "gitbundle-trusted-user-ca-keys.pem"))
	...
	SecretKey = sec.Key("SECRET_KEY").MustString("") // NOTE: This is invalid as we need length = 32 !#@FDEWREWR&*(
	...
	OnlyAllowPushIfGiteaEnvironmentSet = sec.Key("ONLY_ALLOW_PUSH_IF_GITBUNDLE_ENVIRONMENT_SET").MustBool(true)
	...
	if InstallLock && InternalToken == "" {
		// if GitBundle has been installed but the InternalToken hasn't been generated (upgrade from an old release), we should generate
	...
	// The following is a purposefully undocumented option. Please do not run GitBundle as root. It will only cause future headaches.
	...
	if os.Getuid() == 0 {
		if !unsafeAllowRunAsRoot {
			...
			log.Fatal("GitBundle is not supposed to be run as root. Sorry. If you need to use privileged TCP ports please instead use setcap and the `cap_net_bind_service` permission")
		}
		log.Critical("You are running GitBundle using the root user, and have purposely chosen to skip built-in protections around this. You have been warned against this.")
	}
	...
	// Rewrite the default themes, make sure the user defined themes disabled
	UI.Themes = []string{`dark`}
