...
type defaultLogOptions struct {
	...
	flags          string
	filename       string // path.Join(LogRootPath, "gitbundle.log")
	bufferLength   int64
	disableConsole bool
}
...
func newDefaultLogOptions() defaultLogOptions {
	return defaultLogOptions{
		...
		flags:          "stdflags",
		filename:       filepath.Join(LogRootPath, "gitbundle.log"),
		bufferLength:   10000,
...
func generateLogConfig(sec *ini.Section, name string, defaults defaultLogOptions) (mode, jsonConfig, levelName string) {
	...
	case "smtp":
		...
		logConfig["subject"] = sec.Key("SUBJECT").MustString("Diagnostic message from GitBundle")
	}
...
func generateNamedLogger(key string, options defaultLogOptions) *LogDescription {
	...
	for _, name := range sections {
		...
		if err := log.NewNamedLogger(key, options.bufferLength, name, provider, config, IsProd); err != nil {
			// Maybe panic here?
...
func newLogService() {
	log.Info("GitBundle v%s%s", AppVer, AppBuiltWith)
	...
	for _, name := range sections {
		...
		provider, config, levelName := generateLogConfig(sec, name, options)
		log.NewLogger(options.bufferLength, name, provider, config, IsProd)
		description.SubLogDescriptions = append(description.SubLogDescriptions, SubLogDescription{
			Name:     name,
			Provider: provider,
			Config:   config,
		})
		log.Info("GitBundle Log Mode: %s(%s:%s)", util.ToTitleCase(name), util.ToTitleCase(provider), levelName)
	}
