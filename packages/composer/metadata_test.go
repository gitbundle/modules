...
const (
	name        = "gitbundle/composer-package"
	description = "Package Description"
	packageType = "composer-plugin"
	author      = "GitBundle Authors"
	email       = "no.reply@gitbundle.com"
	homepage    = "https://gitbunde.com"
	license     = "MIT"
)
...
const composerContent = `{
    ...
    "homepage": "` + homepage + `",
    "autoload": {
        "psr-4": {"GitBundle\\ComposerPackage\\": "src/"}
    },
    "require": {
        "php": ">=7.2 || ^8.0"
    }
...
func TestParseComposerFile(t *testing.T) {
	...
	t.Run("InvalidPackageVersion", func(t *testing.T) {
		cp, err := ParseComposerFile(strings.NewReader(`{"name": "gitbundle/composer-package", "version": "1.a.3"}`))
		assert.Nil(t, cp)
