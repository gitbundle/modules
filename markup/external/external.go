...
func (p *Renderer) Render(ctx *markup.RenderContext, input io.Reader, output io.Writer) error {
	var (
		...
		command      = strings.NewReplacer(envMark("GITBUNDLE_PREFIX_SRC"), ctx.URLPrefix,
			envMark("GITBUNDLE_PREFIX_RAW"), urlRawPrefix).Replace(p.Command)
	...
	if p.IsInputFile {
		// write to temp file
		f, err := os.CreateTemp("", "gitbundle_input")
		if err != nil {
	...
	cmd.Env = append(
		os.Environ(),
		"GITBUNDLE_PREFIX_SRC="+ctx.URLPrefix,
		"GITBUNDLE_PREFIX_RAW="+urlRawPrefix,
	)
	if !p.IsInputFile {
		cmd.Stdin = input
	}
